...
export default function LabTestDashboard() {
  ...
  const currentTime = new Date().toLocaleString('en-US', {
    ...
    const fetchConfigTests = async () => {
      try {
        ...
        const labRes = await axios.get('http://localhost:5000/dropdown/labtests', {
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      toast({ title: 'Error fetching lab patients', description: e.message, status: 'error' });
    } finally {
    ...
    const loadAll = async () => {
      try {
        const [labRes, patRes] = await Promise.all([
          axios.get('http://localhost:5000/dropdown/labtests',   { headers:{ Authorization:`Bearer ${token}` }}),
          axios.get('http://localhost:5000/lab/patients',        { headers:{ Authorization:`Bearer ${token}` }})
        ]);
        setConfigTests(labRes.data);
  ...
  const handlePatientSelect = async (psrNo) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://localhost:5000/get_patient/${psrNo}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      const processed = patient.lab_tests.map((t) => {
        const cfg = configTests.find(
          ct => ct.test_name.toLowerCase() === t.lab_test.toLowerCase() || ct.test_id === t.lab_test
        );

        if (cfg && cfg.test_id.toLowerCase().startsWith('group')) {
          const names = parseSubTestNames(cfg.test_name);
          const details = names.map(n => {
            const sub = configTests.find(ct => ct.test_name.toLowerCase() === n.toLowerCase());
            return {
  ...
  return (
    <Flex direction="column" h="100vh" bg="gray.50" overflow="hidden">
      {/* HEADER */}
      <Flex as="header" w="100%" h="64px" bg="white" boxShadow="sm" align="center" px="4" position="relative">
        <Flex position="absolute" left="50%" transform="translateX(-50%)" align="center">
          <FiCalendar size={18} />
          <Text ml="2" fontSize="sm" color="gray.600">{currentTime}</Text>
        </Flex>
        <HStack spacing="4" position="absolute" right="4">
          <IconButton icon={<FiBell />} aria-label="Notifications" variant="ghost" size="sm" />
          <IconButton icon={<FiMail />} aria-label="Messages" variant="ghost" size="sm" />
          <Menu>
      ...
      <Box as="main" flex="1" px="8" py="6" mx="8" maxW="1200px" overflow="hidden" overflowY="auto">
        ...
        <Flex mb="8" gap="6" flexWrap="wrap">
          ...
          <Box flex="3" bg="blue.50" borderRadius="2xl" boxShadow="md" p="2">
            <Flex justify="space-between" align="center">
              <Text fontSize="xl" fontWeight="bold" color="gray.800">Test Request Overview</Text>
              <Menu>
                <MenuButton as={Button} rightIcon={<Icon as={FiChevronDown} />} size="sm" variant="outline" borderRadius="full">
                  {timeframe === 'last_month' ? 'Last Month' : 'This Week'}
                </MenuButton>
                <MenuList>
                  <MenuItem onClick={() => setTimeframe('last_month')}>Last Month</MenuItem>
                  <MenuItem onClick={() => setTimeframe('this_week')}>This Week</MenuItem>
                </MenuList>
              </Menu>
            </Flex>
            ...
            <HStack spacing="3" mt="4">
              {['All','Urgent','Pending','In-Progress','Completed'].map(tab => (
                <Button
                  ...
                  _hover={{ bg:'gray.100' }}
                  border="1px solid"
                  borderColor={selectedTab===tab ? 'purple.200' : 'gray.200'}
                >
                  {tab}
                </Button>
              ))}
            ...
            <Grid templateColumns="2fr repeat(2,1fr)" templateRows="repeat(2,1fr)" gap={0} mt="6">
              ...
              >
                <Flex justify="space-between" align="center">
                  <Text fontSize="md" fontWeight="medium" color="blue.600">Laboratory workload</Text>
                  <Flex align="center">
                    <Icon as={FiArrowUp} boxSize="4" color="blue.500" />
                    <Text ml="1" fontSize="sm" color="blue.500">{workloadChange}%</Text>
                  </Flex>
                </Flex>

                <Text fontSize="4xl" fontWeight="bold" color="blue.600" mt="2">{workload}%</Text>
                <Progress value={workload} size="xs" colorScheme="blue" borderRadius="sm" mt="2" />

                <Text fontSize="sm" color="gray.600" mt="3">
                  Laboratory utilization rates have shown an increase—boosting efficiency and resource optimization.
                </Text>
              ...
              >
                <Flex justify="space-between" align="center">
                  <Text fontSize="sm" color="gray.500">Urgent</Text>
                  <Flex align="center">
                    <Icon as={FiArrowDown} boxSize="4" color="red.500" />
                    <Text ml="1" fontSize="xs" color="red.500">{percent.Urgent}%</Text>
                  </Flex>
                </Flex>
                <Text fontSize="2xl" fontWeight="bold" color="orange.400" mt="2">{stats.Urgent}</Text>
              </Box>
              ...
              >
                <Flex justify="space-between" align="center">
                  ...
                  <Flex align="center">
                    ...
                    <Text ml="1" fontSize="xs" color="green.400">{percent.Pending}%</Text>
                  </Flex>
                </Flex>
                <Text fontSize="2xl" fontWeight="bold" color="green.400" mt="2">{stats.Pending}</Text>
              </Box>
              ...
              >
                <Flex justify="space-between" align="center">
                  ...
                  <Flex align="center">
                    ...
                    <Text ml="1" fontSize="xs" color="blue.400">{percent['In-Progress']}%</Text>
                  </Flex>
                </Flex>
                <Text fontSize="2xl" fontWeight="bold" color="blue.400" mt="2">{stats['In-Progress']}</Text>
              </Box>
              ...
              >
                <Flex justify="space-between" align="center">
                  ...
                  <Flex align="center">
                    ...
                    <Text ml="1" fontSize="xs" color="purple.400">{percent.Completed}%</Text>
                  </Flex>
                </Flex>
                <Text fontSize="2xl" fontWeight="bold" color="purple.400" mt="2">{stats.Completed}</Text>
              </Box>
            </Grid>
          </Box>
          ...
          <Box flex="1" display="flex" flexDirection="column">
            ...
            >
              ...
              <Text fontSize="sm" color="gray.600" mb="2">
                Equipment maintenance is due for the Hematology Analyzer. Schedule maintenance to avoid disruptions.
              </Text>
        ...
        <Flex align="center" mb="4">
          <Heading as="h3" size="md" color="gray.800" mr="2">Upcoming Lab Tests</Heading>
          <IconButton aria-label="Refresh" icon={<FiRefreshCw />} variant="ghost" size="sm" onClick={fetchPatients} />
        </Flex>
          ...
          {listLoading ? (
            ...
            <>
              <Flex
                ...
                mb="3"
              >
                <Box w="22%" minW="150px">Name</Box>
                <Box w="16%" minW="120px">PSRN No.</Box>
                <Box w="10%" minW="40px">Age</Box>
                <Box w="10%" minW="40px">Gender</Box>
                <Box w="22%" minW="140px">Visiting Time</Box>
                <Box w="20%" minW="120px">Last Visit</Box>
              </Flex>

              {displayedPatients.length === 0 ? (
                <Flex h="100px" align="center" justify="center">
                  <Text color="gray.500" fontSize="lg">No lab tests right now.</Text>
                </Flex>
              ) : (
                displayedPatients.map((p, i) => (
                  ...
                  >
                    <Box w="22%" minW="150px" display="flex" alignItems="center">
                      <Avatar size="sm" name={p.name} mr="2" />
                      <Text fontSize="sm" color="gray.800">{p.name}</Text>
                    </Box>
                    <Box w="16%" minW="120px" display="flex" alignItems="center">
                    ...
                    <Box w="10%" minW="40px"><Text fontSize="sm" color="gray.600">{p.age}</Text></Box>
                    <Box w="10%" minW="40px"><Text fontSize="sm" color="gray.600">{p.gender}</Text></Box>
                    <Box w="22%" minW="140px"><Text fontSize="sm" color="gray.600">{p.visitingTime||'TBD'}</Text></Box>
                    <Box w="20%" minW="120px"><Text fontSize="sm" color="gray.600">{p.lastVisit||'Nil'}</Text></Box>
                  </Flex>
                ))
              )}
            </>
...
<Modal isOpen={isOpen} onClose={onClose} size="4xl" isCentered motionPreset="scale">
  ...
  <ModalContent bg="white" borderRadius="2xl" boxShadow="2xl" overflow="hidden">
                    ...
                    <Tr key={idx}>
                      ...
                      <Td>
                        <Input
                          ...
                          value={test.result}
                          onChange={e => handleIndividualResultChange(idx, e.target.value)}
                        />
                      </Td>
                      <Td whiteSpace="normal" wordBreak="break-word" color="gray.700">
                ...
                if (test.type === 'group') {
                  return (
                    <React.Fragment key={idx}>
                      <Tr>
                        <Td colSpan={4} bg="blue.50" fontWeight="bold" color="blue.600">
                          {test.lab_test} (Overall: {test.groupReference} {test.groupUnits})
                        </Td>
                      </Tr>
                        ...
                        <Tr key={si}>
                          ...
                          <Td>
                            <Input
                              ...
                              onChange={e => handleSubResultChange(idx, si, e.target.value)}
                            />
                          </Td>
                          <Td whiteSpace="normal" wordBreak="break-word" color="gray.700">
                            {test.subTestDetails[si]?.reference_range ?? 'N/A'}
                          </Td>
                          <Td whiteSpace="normal" wordBreak="break-word" color="gray.700">
                          ...
                          <Tr key={si}>
                            ...
                            <Td>
                              <Input
                                ...
                                onChange={e => handleMultiResultChange(idx, si, e.target.value)}
                              />
                            </Td>
                            <Td whiteSpace="normal" wordBreak="break-word" color="gray.700">
                              {r}
    ...
    <ModalFooter bg="gray.50" px={6} py={4}>
      ...
      <Button colorScheme="green" borderRadius="full" onClick={handlePrint}>
        Print Report
      </Button>
    </ModalFooter>
  </ModalContent>
      ...
      <Modal isOpen={isSuccessOpen} onClose={onSuccessClose} size="md" isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="blue" onClick={onSuccessClose}>Close</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </Flex>
