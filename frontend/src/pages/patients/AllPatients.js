...
export default function AllPatients({ onLogout }) {
  ...
  // Search Past Patients State
  const [searchInstId, setSearchInstId] = useState('');
  ...
  const handleSearchPastPatients = async () => {
    if (!searchInstId.trim() && !searchDate && !searchName.trim()) {
      setSearchError('Please fill in at least one search criterion (Institute ID, Last Visit Date, or Name).');
      return;
    }
    setSearchError('');
    ...
    try {
      ...
      const params = {};
      if (searchInstId.trim()) params.institute_id = searchInstId.trim();
      if (searchDate) params.last_visit_date = searchDate;
      if (searchName.trim()) params.name = searchName.trim();

      const response = await axios.get(`${BASE_URL}/doctor/search_past_patients`, {
        headers: { Authorization: `Bearer ${token}` },
        params,
      });
      setSearchResults(response.data);
    } catch (error) {
      console.error('Error searching past patients:', error);
      setSearchError(error.response?.data?.error || 'Failed to search past patients.');
    } finally {
    ...
    <Flex direction="column" minH="100vh" bg={containerBg}>
      ...
      <Box as="main" flex="1" display="flex" flexDirection="column" p={{ base: 4, md: 6 }}>
        ...
        >
          ...
          <Tabs variant="soft-rounded" colorScheme="blue" display="flex" flexDirection="column" flex="1">
            <TabList mb={4}>
              <Tab fontSize="sm" fontWeight="bold">Recent Visits (Past 7 Days)</Tab>
              <Tab fontSize="sm" fontWeight="bold">Search Past Patients</Tab>
            </TabList>
            ...
            <TabPanels display="flex" flexDirection="column" flex="1">
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {filteredPatients.length === 0 ? (
                          <Tr>
                            <Td colSpan={4} py={10}>
                              <Text fontSize="lg" color="gray.500" textAlign="center">No patient history available.</Text>
                            </Td>
                          </Tr>
                        ) : (
              ...
              {/* Tab 2: Search Past Patients */}
              <TabPanel p={0} display="flex" flexDirection="column" flex="1">
                <Box bg="gray.50" p={4} borderRadius="xl" border="1px solid" borderColor="gray.200" mb={5}>
                  <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4}>
                    <FormControl>
                      <FormLabel fontSize="2xs" fontWeight="bold" color="gray.500">INSTITUTE ID</FormLabel>
                      <Input
                        size="sm"
                        borderRadius="md"
                        bg="white"
                        value={searchInstId}
                        onChange={(e) => setSearchInstId(e.target.value)}
                      />
                    </FormControl>
                    <FormControl>
                      <FormLabel fontSize="2xs" fontWeight="bold" color="gray.500">LAST VISIT DATE</FormLabel>
                      <Input
                        type="date"
                        size="sm"
                        borderRadius="md"
                    ...
                    <FormControl>
                      <FormLabel fontSize="2xs" fontWeight="bold" color="gray.500">PATIENT NAME</FormLabel>
                      <Input
                        size="sm"
                        borderRadius="md"
                        bg="white"
                  ...
                  {searchError && (
                    <Text color="red.500" fontSize="xs" mt={2} fontWeight="medium">{searchError}</Text>
                  )}
                  <Flex justify="flex-end" mt={4}>
                    <Button
                      ...
                      onClick={handleSearchPastPatients}
                    >
                      Search Past Patients
                    </Button>
                  </Flex>
                </Box>
                        ...
                        <Table variant="simple">
                          ...
                          <Tbody>
                            {searchResults.length === 0 ? (
                              <Tr>
                                <Td colSpan={4} py={10}>
                                  <Text fontSize="md" color="gray.500" textAlign="center">No matching past patient records found.</Text>
                                </Td>
                              </Tr>
                            ) : (
